...
function ProductSkeleton() {
  return (
    <div className="group bg-white rounded-3xl border border-[#EBE3D5] p-3 flex flex-col justify-between transition-all">
      <div>
        {/* Shimmer Image Box */}
        <div className="aspect-square bg-[#FAF6F0] rounded-2xl animate-pulse flex items-center justify-center p-3 sm:p-6 relative">
          <div className="w-8 h-8 sm:w-12 sm:h-12 text-[#EBE3D5]/50 font-serif text-2xl sm:text-3xl font-black select-none">B</div>
        </div>

        {/* Shimmer Metadata & Title */}
        <div className="px-2 pt-4 space-y-2">
          <div className="h-2.5 w-12 sm:w-16 bg-[#FBEBE7] rounded animate-pulse"></div>
          <div className="h-4 sm:h-5 w-24 sm:w-40 bg-[#341E19]/10 rounded animate-pulse"></div>
  ...
  return (
    <div className="min-h-screen bg-white selection:bg-[#EB4915] selection:text-white flex flex-col justify-between">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 grow w-full">
        ...
        {isLoading ? (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-8 animate-fade-in">
          ...
          <div className="text-center py-20 bg-[#FAF6F0]/50 rounded-3xl border border-dashed border-[#EBE3D5]">
            <span className="text-4xl block font-sans">🍞</span>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-8">
            {filteredProducts.map((p) => {
              ...
              return (
                <div 
                  key={p.id} 
                  className="group bg-white rounded-3xl border border-[#EBE3D5] p-3 hover:shadow-md transition-all flex flex-col justify-between animate-fade-in"
                >
                  <div>
                    {/* Image container on soft pastel panel */}
                    <div className="aspect-square bg-[#F5F3EE] rounded-2xl overflow-hidden flex items-center justify-center p-3 sm:p-6 relative">
                      <img
                        src={p.img}
                        alt={p.name}
                    ...
                    <div className="px-2 pt-4">
                      ...
                      <h3 className="font-serif font-extrabold text-xs sm:text-base text-[#23110E] mt-0.5 leading-tight line-clamp-1 sm:line-clamp-none">
                        {p.name}
                      </h3>
                      <p className="text-[10px] sm:text-[11px] text-[#341E19]/70 mt-1.5 leading-relaxed line-clamp-2">
                        {p.desc}
                      </p>
                    </div>
                  </div>

                  <div className="px-2 pt-3 sm:pt-4 mt-3 sm:mt-4 border-t border-[#EBE3D5]/60 flex items-center justify-between gap-1">
                    <div>
                      <span className="block text-[8px] sm:text-[9px] text-[#341E19]/40 font-bold uppercase leading-none">
                        Price
                      </span>
                      <span className="text-xs sm:text-sm font-extrabold text-[#EB4915]">
                        {formatCurrency(p.price)}
                      </span>
                    </div>
                    <button
                      onClick={() => addToCart(p)}
                      className={`text-[9px] font-bold uppercase tracking-wider px-2.5 py-2 sm:px-4 sm:py-2.5 rounded-full transition-all flex items-center gap-1 sm:gap-1.5 cursor-pointer shadow-sm shrink-0 ${
                        isAdded 
